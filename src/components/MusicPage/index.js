...
class MusicPage extends Component {
  ...
  renderNoSongsFoundView = () => (
    <div className="no-songs-container">
      <p className="no-songs-para">No Songs Found</p>
    </div>
  )

  onClickDeleteTrack = id => {
    const {playList} = this.state
    const updatedPlayList = playList.filter(eachTrack => eachTrack.id !== id)
      ...
      <div className="main-container">
        ...
        <ul className="list-style">
          {searchResults.length === 0
            ...
            : searchResults.map(eachSongTrack => (
                <PlayList
                  key={eachSongTrack.id}
                  trackDetails={eachSongTrack}
